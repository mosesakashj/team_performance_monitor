...
Doing both together in SQL means nesting a recursive CTE inside a self-join — legal, but slow and hard to read. In Cypher, it's one continuous pattern match (see [`staffing.queries.js`](server/src/queries/staffing.queries.js)).
...
/
  server/            Express API
    ...
    scripts/
      seed.js         idempotent, MERGE-based seed loader
...
npm run seed          # loads 280 nodes / ~2,450 relationships; safe to re-run
```
...
1. **Staffing candidates** ([`staffing.queries.js`](server/src/queries/staffing.queries.js)) — the flagship query. Multi-hop (`RELATED_TO*0..2` skill closure) **and** relational-awkward (combined with a `WORKED_ON<->WORKED_ON` collaboration self-join in one pass). Powers the "Recommended Candidates" panel on each project.
2. **Shortest path between two people** ([`people.queries.js`](server/src/queries/people.queries.js)) — relational-awkward: a variable-length `shortestPath()` across three mixed relationship types (`WORKED_ON|MEMBER_OF|ENDORSED`). Powers the Connection Finder page.
3. **Collaboration network** ([`people.queries.js`](server/src/queries/people.queries.js)) — an explicit 2-hop traversal (colleagues, then colleagues-of-colleagues) via shared `WORKED_ON` projects. Powers a person's network view.
...
- The driver's `maxTransactionRetryTime` is deliberately capped at 2s (the default is 30s) so a DB outage surfaces as a fast, clean 503 instead of a multi-second hang. This was caught by actually running the app against an unreachable database during development, not assumed.
- Every query error is normalized to `{ error: { message, code } }` with a 503 status by [`db/driver.js`](server/src/db/driver.js) — route handlers don't need their own try/catch for connectivity issues.
...
- No auth/authorization layer — this is a read-only demo of the data model and query patterns.
